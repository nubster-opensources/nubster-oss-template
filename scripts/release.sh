#!/usr/bin/env bash
#
# scripts/release.sh: opinionated release preparation wrapper for {{project-name}}.
#
# Usage:
...
#
# Required secrets: CARGO_REGISTRY_TOKEN (crates.io publish token).
...
# TODO: update the link refs below to match your repository URL.
unreleased_link_re = re.compile(r"^\[Unreleased\]: .*$", re.MULTILINE)
new_links = (
    "[Unreleased]: https://github.com/nubster-opensources/{{project-name}}/compare/v"
    + new_version
    + "...HEAD\n["
    + new_version
    + "]: https://github.com/nubster-opensources/{{project-name}}/releases/tag/v"
    + new_version
)
...
The release workflow then publishes the crates to crates.io and creates the GitHub Release from the CHANGELOG section.
EOF
)
